...
func (inst *inst) testRepro() ([]byte, error) {
	...
	if err == nil && len(inst.reproC) > 0 {
		...
		out, err = transformError(execProg.RunCProgRaw(inst.reproC, inst.cfg.Target,
			inst.cfg.Timeouts.NoOutput/2, nil))
	}
	return out, err
}
...
func FuzzerCmd(args *FuzzerCmdArgs) string {
	...
	if args.Optional != nil {
		flags := []tool.Flag{
			...
			{Name: "enable_nvidia", Value: fmt.Sprint(args.Optional.EnableNvidia)},
			{Name: "enable_opencl", Value: fmt.Sprint(args.Optional.EnableOpenCL)},
			{Name: "enable_vulkan", Value: fmt.Sprint(args.Optional.EnableVulkan)},
		}
