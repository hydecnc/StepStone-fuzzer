...
func (inst *ExecProgInstance) RunCProg(p *prog.Prog, duration time.Duration,
	...
	return inst.RunCProgRaw(src, p.Target, duration, p.Calls)
}

func (inst *ExecProgInstance) RunCProgRaw(src []byte, target *prog.Target,
	duration time.Duration, calls []*prog.Call) (*RunResult, error) {
	bin, err := csource.BuildNoWarn(target, calls, src)
	if err != nil {
		return nil, err
	}
	defer os.Remove(bin)
