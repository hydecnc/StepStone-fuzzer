...
func TestCudaCProg(t *testing.T) {
	opts := Options{
		Threaded:     false,
		Repeat:       false,
		Procs:        1,
	...
	testcase := "syz_cuDevicePrimaryCtxReset(0x0)"
	target, _ := prog.GetTarget(targets.Linux, targets.AMD64)
	p, _ := target.Deserialize([]byte(testcase), prog.Strict)
	testOne(t, p, opts)
}
...
func testOne(t *testing.T, p *prog.Prog, opts Options) {
	...
	bin, err := Build(p.Target, p.Calls, src)
	if err != nil {
		if atomic.AddUint32(&failedTests, 1) > maxFailures {
			t.Fatal()
		}
