...
func candidateRequest(input Candidate) *Request {
	...
	input.Prog.ParentID = -1
	input.Prog.DoneSegvMutate = false
	return &Request{
		Prog:       input.Prog,
		NeedSignal: rpctype.NewSignal,
		stat:       statCandidate,
...
type bypassSegvFaultJob struct {
	p           *prog.Prog
	segvCallIdx int
	info        ipc.CallInfo
	flags       ProgTypes
...
func (job *bypassSegvFaultJob) priority() priority {
	return priority{bypassSegvFaultPrio}
}

func (job *bypassSegvFaultJob) run(fuzzer *Fuzzer) {
	log.Logf(2, "[MAGIC!?] Bypass sgev fault in the prog %d:", job.p.ID)

	const iters = 30
	rnd := fuzzer.rand()
	for i := 0; i < iters; i++ {
		p := job.p.Clone()
		log.Logf(2, "[MAGIC!?] Mutate sgev fault in the prog %d call %d:", job.p.ID, job.segvCallIdx)
		p.MutateSegvFaultCall(rnd, prog.RecommendedCalls,
			job.segvCallIdx,
			fuzzer.ChoiceTable(),
			fuzzer.Config.NoMutateCalls,
			fuzzer.Config.Corpus.Programs())
		p.ParentID = job.p.ID
		p.DoneSegvMutate = true
...
func (job *triageJob) minimize(fuzzer *Fuzzer, newSignal signal.Signal) (stop bool) {
	...
	job.p, job.call = prog.Minimize(job.p, job.call, false,
		func(p1 *prog.Prog, call1 int) bool {
			...
			p1.ParentID = job.p.ID
			p1.DoneSegvMutate = false
...
func (job *smashJob) faultInjection(fuzzer *Fuzzer) {
	for nth := 1; nth <= 100; nth++ {
		...
		newProg.ParentID = job.p.ID
		newProg.DoneSegvMutate = false
		result := fuzzer.exec(job, &Request{
			Prog: job.p,
			stat: statSmash,
		})
