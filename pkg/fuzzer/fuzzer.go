...
func (fuzzer *Fuzzer) checkSegvFault(info *ipc.ProgInfo) bool {
	if info == nil {
		return false
	}

	for i, inf := range info.Calls {
		log.Logf(2, "[MAGIC!?] checkSegvFault prog %d call %d executed %d %v", info.ProgID, i, inf.Flags&ipc.CallSegvFault, inf.Errno)
		if inf.Flags&ipc.CallSegvFault != 0 {
			// Segv fault happened
			return true
		}
	}
...
func (fuzzer *Fuzzer) Done(req *Request, res *Result) {
	...
	if res.Info != nil {
		req.Prog.ID = res.Info.ProgID
		req.Prog.ParentID = res.Info.ProgParentID
		req.Prog.DoneSegvMutate = res.Info.DoneSegvMutate
	}
	if fuzzer.Config.EnableSegvMutation && fuzzer.checkSegvFault(res.Info) && rs.Intn(10) == 0 {
		if !req.Prog.DoneSegvMutate {
			fuzzer.SegvBypassAttempt(req.Prog, res.Info, req.flags)
		}
	} else {
...
func (fuzzer *Fuzzer) SegvBypassAttempt(p *prog.Prog, info *ipc.ProgInfo, flags ProgTypes) {
	for i, inf := range info.Calls {
		if inf.Flags&ipc.CallSegvFault != 0 {
			// Segv fault happened
			log.Logf(2, "[MAGIC!?] found segv fault in prog %d", p.ID)
			fuzzer.startJob(&bypassSegvFaultJob{
				p:           p.Clone(),
				segvCallIdx: i,
				info:        inf,
				flags:       flags,
