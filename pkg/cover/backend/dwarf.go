...
func processModule(params *dwarfParams, module *Module, info *symbolInfo,
	...
	log.Logf(0, "[MAGIC!? processModule: %v] (%v)", module.Name, module.Path)
	if err != nil {
		return nil, err
	}
...
func readTextRanges(debugInfo *dwarf.Data, module *Module, pcFix pcFixFn) (
	...
	for r := debugInfo.Reader(); ; {
		...
		if ent.Tag != dwarf.TagCompileUnit {
			log.Logf(0, "found unexpected tag %v on top level (%v)", ent.Tag, module.Name)
			continue
			//return nil, nil, fmt.Errorf("found unexpected tag %v on top level (%v)", ent.Tag, module.Name)
		}
