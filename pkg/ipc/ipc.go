...
const (
	...
	FlagEnableNvidia                             // setup nvidia driver
	FlagEnableOpenCL                             // setup opencl fuzzing
	FlagEnableVulkan                             // setup vulkan fuzzing
)
...
const (
	...
	CallSegvFault                           // call caused a segv fault
)
...
func FlagsToSandbox(flags EnvFlags) string {
	if flags&FlagSandboxNone != 0 {
		return "none"
	} else if flags&FlagSandboxSetuid != 0 {
		return "setuid"
...
func (env *Env) parseOutput(p *prog.Prog, opts *ExecOpts) (*ProgInfo, error) {
	...
	info := &ProgInfo{Calls: make([]CallInfo, len(p.Calls)), ProgID: p.ID, ProgParentID: p.ParentID, DoneSegvMutate: p.DoneSegvMutate}
...
func makeCommand(pid int, bin []string, config *Config, inFile, outFile *os.File, outmem []byte,
	...
	log.Logf(2, "[MAGIC!?] execute %v", bin)
...
func (c *command) exec(opts *ExecOpts, progData []byte) (output []byte, hanged bool, err0 error) {
	...
	log.Logf(1, "[MAGIC!?] exec flag: %v   programTimeout: %v ms   slowdown: %v", req.execFlags, req.programTimeoutMS, c.config.Timeouts.Slowdown)
	reqData := (*[unsafe.Sizeof(*req)]byte)(unsafe.Pointer(req))[:]
	if _, err := c.outwp.Write(reqData); err != nil {
		output = <-c.readDone
