...
func (p *Prog) MutateSegvFaultCall(rs rand.Source, ncalls int, segvCallIdx int, ct *ChoiceTable, noMutate map[int]bool, corpus []*Prog) {
	r := newRand(p.Target, rs)
	if ncalls < len(p.Calls) {
		ncalls = len(p.Calls)
	}
	...
	for stop, ok := false, false; !stop; stop = ok && len(p.Calls) != 0 && r.oneOf(3) {
		...
		case r.oneOf(5):
			// Not all calls have anything squashable,
			// so this has lower priority in reality.
			ok = ctx.squashAnyPick(segvCallIdx)
		default:
			ok = ctx.mutateArgPick(segvCallIdx)
		}
	}
	p.sanitizeFix()
	p.debugValidate()
...
func (ctx *mutator) squashAnyPick(callIdx int) bool {
	p, r := ctx.p, ctx.r
	complexPtrs := p.complexPtrsPick(callIdx)
	if len(complexPtrs) == 0 {
		return false
	}
	ptr := complexPtrs[r.Intn(len(complexPtrs))]
...
func (ctx *mutator) mutateArgPick(callIdx int) bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) == 0 {
		return false
	}
