...
type Config struct {
	...
	PCIVideoDevice []string `json:"pci_video_device"`
	PCIAudioDevice []string `json:"pci_audio_device"`
...
func (pool *Pool) ctor(workdir, sshkey, sshuser string, index int) (vmimpl.Instance, error) {
	inst := &instance{
		...
		vga_id:     pool.cfg.PCIVideoDevice[index],
		adu_id:     pool.cfg.PCIAudioDevice[index],
	}
	log.Logf(0, "Booting vm %v with GPU %v", index, pool.cfg.PCIVideoDevice[index])
...
func (inst *instance) boot() error {
	...
	if inst.vga_id != "" {
		args = append(args,
			"-device", fmt.Sprintf("vfio-pci,host=%v,multifunction=on,x-vga=on", inst.vga_id),
		)
	}
	if inst.adu_id != "" {
		args = append(args,
			"-device", fmt.Sprintf("vfio-pci,host=%v", inst.adu_id),
		)
	}
	if inst.debug {
