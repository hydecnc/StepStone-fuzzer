...
/*func (mgr *Manager) PrepareDumpPC() {
	var pcs []uint64
	rg, err := getReportGenerator(mgr.cfg, mgr.modules)
	if err != nil {
		log.Logf(0, "fail to PrepareDumpPC")
		return
	}
	...
	for _, inp := range items {
		...
		pcs = append(pcs, coverToPCs(rg, inp.Cover)...)
	}
	mgr.dumpCovPC(pcs)

}

func (mgr *Manager) dumpCovPC(PCs []uint64) {

	now := time.Now()
	timestamp := now.Format("2006_01_02_15-04")
	fileName := fmt.Sprintf("cov-%s.out", timestamp)
	filePath := filepath.Join(mgr.cfg.Workdir, fileName)
	file, err := os.OpenFile(filePath, os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Logf(0, "Error opening file: %v\n", err)
		return
	}
	defer file.Close()
	...
	for _, pc := range PCs {
		if pc > 0xffffffffa0000000 && pc < 0xffffffffb0000000 {
			line := strconv.FormatUint(pc, 16) + "\n"
			_, err = file.WriteString(line)
			if err != nil {
				log.Logf(0, "Error writing to file: %v\n", err)
			}
		}
	}
}*/
