...
func RunManager(cfg *mgrconfig.Config) {
	...
	go func() {
		...
		for lastTime := time.Now(); ; {
			...
			/*if time.Since(lastDump) >= 15*time.Minute {
				mgr.PrepareDumpPC()
				lastDump = time.Now()
			}*/
		}
...
func (mgr *Manager) runInstanceInner(index int, instanceName string) (*report.Report, []byte, error) {
	...
	if mgr.cfg.GPUVendor == "nvidia" {
		args.Optional.EnableNvidia = true
		args.Optional.EnableOpenCL = true
		args.Optional.EnableVulkan = true
	}
	if mgr.cfg.GPUVendor == "amd" {
		args.Optional.EnableOpenCL = true
		args.Optional.EnableVulkan = true
	}
...
func (mgr *Manager) machineChecked(a *rpctype.CheckArgs, enabledSyscalls map[*prog.Syscall]bool) {
	...
	fuzzerObj := fuzzer.NewFuzzer(context.Background(), &fuzzer.Config{
		...
		EnableSegvMutation: mgr.cfg.EnableSegvMutation,
