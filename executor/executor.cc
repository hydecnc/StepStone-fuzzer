...
const int kMaxArgs = 15;
const int kCoverSize = 2 << 20;
...
const int kMaxOutputComparisons = 16 << 20; // executions with comparsions enabled are usually < 1% of all executions
const int kMaxOutputCoverage = 16 << 20; // coverage is needed in ~ up to 1/3 of all executions (depending on corpus rotation)
const int kMaxOutputSignal = 4 << 20;
const int kMinOutput = 2 << 20; // if we don't need to send signal, the output is rather short.
...
static bool flag_sandbox_bypass;
...
const int kMaxInput = 2 << 20; // keep in sync with prog.ExecBufferSize
...
const uint32 call_flag_segv_fault = 1 << 4;
...
#endif
	else if (flag_sandbox_bypass)
		status = do_sandbox_bypass();
	else
...
void parse_env_flags(uint64 flags)
{
	...
	flag_delay_kcov_mmap = flags & (1 << 15);
	flag_nic_vf = flags & (1 << 16);
	flag_nvidia = flags & (1 << 17);
}
...
void execute_one()
{
	...
	if (cover_collection_required()) {
		if (!flag_threaded) {
#if DEBUG_COV
			debug("enable kcov in execute_one at pid: %d\n", getpid());
#endif
...
void write_coverage_signal(cover_t* cov, uint32* signal_count_pos, uint32* cover_count_pos)
{
...
#if DEBUG_COV
	debug("Enter write_coverage_signal cov->data: 0x%llx\n", (long long unsigned int)cov->data);
#endif
	...
	if (flag_collect_cover) {
...
#if DEBUG_COV
			uint32 last_pc = cover_data[0] + cov->pc_offset;
			uint32 count = 0;
			for (uint32 i = 0; i < cover_size; i++) {
				uint32 pc = cover_data[i] + cov->pc_offset;
				if (last_pc == pc)
					count++;
				else {
					//debug("0x%lx -> %d\n", (long unsigned int)last_pc, count);
					count = 0;
					last_pc = pc;
				}
			}
		...
		for (uint32 i = 0; i < cover_size; i++) {
			unsigned long long pc = cover_data[i] + cov->pc_offset;
#if DEBUG_COV
			if (pc > 0xffffffffa0000000 && pc < 0xffffffffb0000000)
				debug("Found Additional PC: 0x%llx\n", (unsigned long long)(pc));
#endif
			write_output(pc);
		}
...
void write_call_output(thread_t* th, bool finished)
{
	...
	uint32 call_flags = call_flag_executed | (blocked ? call_flag_blocked : 0) | (segv_fault_occurred ? call_flag_segv_fault : 0);
	if (finished) {
...
void* worker_thread(void* arg)
{
	...
	current_thread = th;
	if (cover_collection_required()) {
		debug("enable kcov in worker_thread\n");
...
void execute_call(thread_t* th)
{
	...
	if (flag_coverage)
		cover_reset(&th->cov);
#if DEBUG_COV
	debug("coverage reset before execute_call\n");
	// sleep(5);
#endif
...
#if DEBUG
	debug("[%llums] finish execute_call at pid: %d\n", current_time_ms() - start_time_ms, getpid());
#endif
