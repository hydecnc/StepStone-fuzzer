...
static uint32 segv_fault_occurred = 0;
...
#if SYZ_THREADED || SYZ_REPEAT || SYZ_SANDBOX_NONE || SYZ_SANDBOX_SETUID || SYZ_SANDBOX_NAMESPACE || SYZ_SANDBOX_ANDROID || SYZ_SANDBOX_BYPASS
#if SYZ_THREADED
...
int main(void)
{
	...
	for (procid = 0; procid < /*{{{PROCS}}}*/; procid++) {
		if (fork() == 0) {
			...
			use_temporary_dir();
#endif
#if SYZ_NVIDIA && !(SYZ_THREADED || SYZ_REPEAT || SYZ_SANDBOX_NONE || SYZ_SANDBOX_SETUID || SYZ_SANDBOX_NAMESPACE || SYZ_SANDBOX_ANDROID || SYZ_SANDBOX_BYPASS)
			/*{{{NVIDIA_SETUP}}}*/
#endif
			/*{{{SANDBOX_FUNC}}}*/
