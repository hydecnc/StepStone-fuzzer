...
func main() {
	...
	log.Logf(0, "enable %v  disable %v", *flagEnable, *flagDisable)
...
func createConfig(target *prog.Target, features *host.Features, featuresFlags csource.Features) (
	*ipc.Config, *ipc.ExecOpts) {
	...
	if featuresFlags["nvidia"].Enabled && features[host.FeatureNvidia].Enabled {
		config.Flags |= ipc.FlagEnableNvidia
	}
	if featuresFlags["opencl"].Enabled && features[host.FeatureOpenCL].Enabled {
		config.Flags |= ipc.FlagEnableOpenCL
	}
	if featuresFlags["vulkan"].Enabled && features[host.FeatureVulkan].Enabled {
