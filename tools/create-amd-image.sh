...
DEBOOTSTRAP_PARAMS="--arch=$DEBARCH --include=$PREINSTALL_PKGS --components=main,restricted,universe,multiverse $RELEASE $DIR http://archive.ubuntu.com/ubuntu/"
if [ $FOREIGN = "true" ]; then
...
if [ $DEBARCH == "riscv64" ]; then
    DEBOOTSTRAP_PARAMS="--exclude firmware-atheros $DEBOOTSTRAP_PARAMS http://archive.ubuntu.com/ubuntu/"
fi
...
printf 'network:\n version: 2\n renderer: networkd\n ethernets:\n  eth0:\n   dhcp4: true\n' | sudo tee -a $DIR/etc/netplan/01-netcfg.yaml
sudo chmod 400 $DIR/etc/netplan/01-netcfg.yaml
sudo chroot $DIR /bin/bash -c "systemctl enable systemd-networkd"
