...
func createIPCConfig(features *host.Features, config *ipc.Config) {
	...
	if features[host.FeatureNvidia].Enabled {
		config.Flags |= ipc.FlagEnableNvidia
	}
	if features[host.FeatureOpenCL].Enabled {
		config.Flags |= ipc.FlagEnableOpenCL
	}
	if features[host.FeatureVulkan].Enabled {
		config.Flags |= ipc.FlagEnableVulkan
	}
}
...
func main() {
	...
	var (
		...
		flagNvidia        = flag.Bool("enable_nvidia", false, "Enable Nvidia driver fuzzing")
	)
	defer tool.Init()()
	...
	log.Logf(0, "[MAGIC!?] sandbox: %v", sandbox)
	if *flagTest {
	...
	if *flagNvidia {
		_, err = osutil.RunCmd(20*time.Second, "", "nvidia-persistenced", "--persistence-mode", "--uvm-persistence-mode")
		if err != nil {
			log.Logf(1, "Fail to enable nvidia persistence mode")
		} else {
			log.Logf(1, "Enabled nvidia persistence mode")
		}

		_, err = osutil.RunCmd(20*time.Second, "", "nvidia-modprobe", "-u")
		if err != nil {
			log.Logf(1, "Fail to enable load kernel modules through nvidia-modprobe")
		}
	}
	...
	if r.CheckResult.Features[host.FeatureNvidia].Enabled {
		if !*flagNvidia {
			log.SyzFatalf("Fail to launch Nvidia driver fuzzing, set \"GPUVendor\" to \"nvidia\"")
		}
	}
	...
	_, err = osutil.RunCmd(5*time.Second, "", "sysctl", "-w", "kernel.panic_on_warn=1")
	if err != nil {
		log.Logf(1, "Fail to enable panic_on_warn")
	}
...
func (tool *FuzzerTool) exchangeDataCall(needProgs int, results []executionResult,
	...
	for _, req := range r.Requests {
		...
		p.ID = req.ProgID
		p.ParentID = req.ProgParentID
		p.DoneSegvMutate = req.DoneSegvMutate
