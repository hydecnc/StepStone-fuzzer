...
func (proc *Proc) loop() {
	...
	for {
		...
		log.Logf(2, "[MAGIC!?] executing program %d segv status: %v", req.prog.ID, req.prog.DoneSegvMutate)
		log.Logf(2, "[MAGIC!?] prog %d comes from %d", req.prog.ID, req.prog.ParentID)
...
func (proc *Proc) logProgram(opts *ipc.ExecOpts, p *prog.Prog) {
	...
	case OutputStdout:
		...
		fmt.Printf("%02v:%02v:%02v executing program %v (Prog ID: %d):\n%s\n",
